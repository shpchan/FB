...
(function ($, learun) {
    ...
    // 获取自定义数据 url key valueId key2 valueId2
    clientAsyncData.custmerData2 = {
        states: {},
        get: function (op) {
            if (clientAsyncData.custmerData.states[op.url] == undefined || clientAsyncData.custmerData.states[op.url] == loadSate.no) {
                clientAsyncData.custmerData.states[op.url] = loadSate.ing;
            ...
            if (clientAsyncData.custmerData.states[op.url] == loadSate.ing) {
                ...
                var data = clientData[op.url] || [];
                if (!!data) {
                    op.callback(clientAsyncData.custmerData.find(op.key, op.keyId, op.key2, op.keyId2, data) || {}, op);
                } else {
                    op.callback({}, op);
                }
            }
        ...
        gets: function (op) {
            ...
            if (clientAsyncData.custmerData.states[op.url] == loadSate.ing) {
                ...
                if (!!data) {
                    ...
                    var _text = []
                    $.each(keyList, function (_index, _item) {
                        var _item = clientAsyncData.custmerData.find(op.key, op.keyId, op.key2, op.keyId2, data) || {};
                        if (_item[op.textId]) {
                            _text.push(_item[op.textId]);
                        }
        ...
        find: function (key, keyId, key2, keyId2, data) {
            var res = {};
            for (var i = 0, l = data.length; i < l; i++) {
                if (data[i][keyId] == key && data[i][keyId2] == key2) {
                    res = data[i];
                    break;
                }
            }
    ...
    //可视化看板
    clientAsyncData.desktop1 = {
        states: loadSate.no,
        init: function () {
            if (clientAsyncData.desktop1.states == loadSate.no) {
                clientAsyncData.desktop1.states = loadSate.ing;
                var j = storage.get("desktopData").ver || "";
                learun.httpAsync("GET",
                    top.$.rootUrl + "/LR_Desktop/DTTarget/GetMap1",
                    {
                        ver: j
                    },
                    function (k) {
                        if (!k) {
                            clientAsyncData.desktop1.states = loadSate.fail;
                        } else {
                            if (k.ver) {
                                storage.set("desktopData", k);
                            }
                            clientAsyncData.desktop1.states = loadSate.yes;
                            clientAsyncData.department.init();
                        }
                    });
            }
        }, get: function (k) {
            clientAsyncData.desktop1.init();
            if (clientAsyncData.desktop1.states == loadSate.ing) {
                setTimeout(function () {
                    clientAsyncData.desktop1.get(k);
                }, 100)
            } else {
                var j = storage.get("desktopData").data || {};
                k.callback(j || {}, k);
