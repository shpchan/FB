...
namespace Learun.DataBase.Repository
{
    ...
    public class DbFactory
    {
        ...
        /// <param name="name">连接配置名称</param>
        /// <returns></returns>
        public static IMongoDB GetIMGDatabase(string name)
        {
            string providerName = ConfigurationManager.ConnectionStrings[name].ProviderName;
            return GetIMGDatabaseByIoc(GetDbType(providerName).ToString(), name);
        }
        /// <summary>
        /// 连接数据库
        /// </summary>
        ...
        /// <param name="connString">连接字符串</param>
        /// <returns></returns>
        private static IMongoDB GetIMGDatabaseByIoc(string name, string connString)
        {
            return new Mongo.MongoDB(connString);
        }
        /// <summary>
        /// 获取数据库类型
        /// </summary>
