...
namespace Learun.Application.TwoDevelopment.LR_Desktop
{
    ...
    /// 描 述：图标配置
    /// </summary>
    public class DTChartService1 : RepositoryFactory
    {
        #region  获取数据
        ...
        public IEnumerable<DTChartEntity1> GetPageList1(Pagination pagination, string queryJson)
        {
            try
            {
                ...
                t.F_DataSourceId
                ");
                strSql.Append("  FROM LR_DT_Chart1 t ");
                ...
                if (!string.IsNullOrEmpty(queryJson))
                {
                    ...
                    if (pagination == null)
                    {
                        return this.BaseRepository().FindList<DTChartEntity1>(strSql.ToString(), dp);
                    }
                    else
                    {
                        return this.BaseRepository().FindList<DTChartEntity1>(strSql.ToString(), dp, pagination);
                    }
        ...
        public DTChartEntity1 GetLR_DT_ChartEntity(string keyValue)
        {
            try
            {
                return this.BaseRepository().FindEntity<DTChartEntity1>(keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public void DeleteEntity(string keyValue)
        {
            try
            {
                this.BaseRepository().Delete<DTChartEntity1>(t => t.F_Id == keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        internal List<NameValueCollection> GetSqlData(string id)
        {
            ...
            try
            {
                var chartEntity = this.BaseRepository().FindEntity<DTChartEntity1>(p => p.F_Id == id);
