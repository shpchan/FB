...
namespace Learun.Application.TwoDevelopment.LR_LGManager
{
    ...
    /// 描 述：设备分组维护
    /// </summary>
    public class macgroupService : RepositoryFactory
    {
        #region  获取数据
        ...
        public IEnumerable<tb_macgroup_infoEntity> GetPageList(Pagination pagination, string queryJson)
        {
            try
            {
                ...
                t.gis_visual
                ");
                strSql.Append("  FROM tb_macgroup_info t ");
                strSql.Append("  WHERE 1=1 ");
                var queryParam = queryJson.ToJObject();
                // 虚拟参数
                var dp = new DynamicParameters(new { });
                return this.BaseRepository("BaseDb").FindList<tb_macgroup_infoEntity>(strSql.ToString(),dp, pagination);
            }
            catch (Exception ex)
            {
        ...
        public tb_macgroup_infoEntity Gettb_macgroup_infoEntity(string keyValue)
        {
            try
            {
                return this.BaseRepository("BaseDb").FindEntity<tb_macgroup_infoEntity>(keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public void DeleteEntity(string keyValue)
        {
            try
            {
                this.BaseRepository("BaseDb").Delete<tb_macgroup_infoEntity>(t=>t.id == keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
